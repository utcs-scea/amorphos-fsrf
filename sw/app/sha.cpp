...
int main(int argc, char *argv[]) {
	...
	for (uint64_t app = 0; app < num_apps; ++app) {
		...
		aos[app]->aos_cntrlreg_write(0x28, configs[0].rd_credits);
		aos[app]->aos_cntrlreg_write(0x30, configs[0].num_words);
	}
	
	// end runs
	util.finish_runs(aos, end, 0x38, true, configs[0].num_words);
	...
	for (uint64_t app = 0; app < num_apps; ++app) {
		for (uint64_t i = 0; i < 4; ++i) {
			uint64_t temp;
			aos[app]->aos_cntrlreg_read(0x08*i, temp);
			configs[app].abcdefgh[2*i] = temp & 0xFFFFFFFF;
			configs[app].abcdefgh[2*i+1] = temp >> 32;
		}
		for (uint64_t i = 0; i < 8; ++i) {
			printf("%u ", configs[app].abcdefgh[i]);
		}
		printf("\n");
	}
	*/
	...
	util.print_stats("sha", app_bytes, start, end);
